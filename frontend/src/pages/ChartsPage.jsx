...
export default function ChartsPage() {
  ...
  const availableYears = useMemo(() => {
    ...
    fuelData.forEach((item) => {
      const year = new Date(item.tankkauspva).getFullYear();
      years.add(year);
    });
  ...
  const timeframeOptions = useMemo(() => {
    const options = [{ value: 'all', label: 'Kaikki' }];
  ...
  const filteredFuelDataByCar = useMemo(() => {
    return selectedCar
      ? fuelData.filter((item) => item.rekisteritunnus === selectedCar)
      : fuelData;
  }, [fuelData, selectedCar]);

  const filteredFuelDataByTimeframe = useMemo(() => {
    if (selectedTimeframe === 'all') {
      return filteredFuelDataByCar;
    } else {
      const selectedYear = parseInt(selectedTimeframe, 10);
      return filteredFuelDataByCar.filter((item) => {
        const year = new Date(item.tankkauspva).getFullYear();
        return year === selectedYear;
      });
    }
  }, [filteredFuelDataByCar, selectedTimeframe]);

  const sortedFuelData = useMemo(() => {
    return [...filteredFuelDataByTimeframe].sort((a, b) => new Date(a.tankkauspva) - new Date(b.tankkauspva));
  }, [filteredFuelDataByTimeframe]);
  ...
  return (
    <div>
      ...
      <FormControl style={{ marginBottom: '20px', width: '200px' }}>
        <InputLabel id="timeframe-select-label">Valitse ajanjakso</InputLabel>
        <Select
